...
class PaymentCallbackController extends Controller
{
    public function __construct(
        private EgyptianPaymentGatewayService $paymentService
    ) {}

    /**
     * Handle payment-gateway webhooks.
     */
    public function handleCallback(Request $request, string $gateway): \Illuminate\Http\JsonResponse
    {
        ...
        $success = $this->paymentService->verifyPayment($reference, $enrollmentId);

        return response()->json(['success' => $success]);
    }

    /**
     * Handle return URL from payment gateway (redirects to Flutter app).
     */
    public function returnUrl(Request $request, string $gateway): \Illuminate\Http\RedirectResponse
    {
        ...
        $deepLink = "{$scheme}://payment/result?" . http_build_query([
            'success'       => $success,
